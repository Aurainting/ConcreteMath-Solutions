...
\begin{align*}
  ...
  \Rightarrow \ \text{原式} \ &= H_{2n+1} - \frac{1}{2} H_n
\end{align*}

\begin{problem}{6.5}{1}
  说明怎样从
  \[
    U_n(x,y) = \sum_{k \ge 1} \binom{n}{k} \frac{(-1)^{k-1}}{k} (x+ky)^n \ , \quad \text{整数} \ n \ge 0
  \]
...
\begin{align*}
  ...
  \text{由公式：} \ & \ \boxed{\binom{n}{k} = \binom{n-1}{k} + \binom{n-1}{k-1}} \\
...
\begin{align*}
  ...
  \text{由公式：} \ & \ \boxed{x^n = \sum_k \genfrac{\{}{\}}{0pt}{} {n}{k} x^{\overline{k}}} \\
...
因此：
\begin{align*}
  &\phantom{\Rightarrow} \sum_{k \ge 1} \binom{n}{k} (-1)^{k-1} (x+ky)^{n-1} = x^{n-1} \\
...
两个月后：2 （大兔 \( \times \) 1 + 小兔 \( \times \) 1）

三个月后：3 （大兔 \( \times \) 2 + 小兔 \( \times \) 1）

四个月后：5 （大兔 \( \times \) 3 + 小兔 \( \times \) 2）
...
\begin{enumerate}
  \item[1.] 在 \( F_{n+k} = F_k F_{n+1} + F_{k-1} F_n \) 中，令 \( k = 1-n \) 则：\( F_1 = F_{1-n} F_{n+1} + F_{-n} F_n \)，利用公式：\( F_{-n} = (-1)^{n-1} F_n \)，\( n \) 是整数。则 \( 1 = (-1)^{n-2} F_{n-1} F_{n+1} + (-1)^{n-1} F_n^2 \)，有 \( (-1)^n = F_{n-1} F_{n+1} - F_n^2 \)
  \item[2.] 令 \( m = 1, k = n-1 \) 则：
    \begin{align*}
      &\phantom{=} K_{1+n}(x_1, \cdots, x_{1+n}) K_{n-1}(x_2, \cdots, x_n) \\
      &= K_n(x_1, \cdots, x_n) K_n(x_2, \cdots, x_{1+n}) + (-1)^{n-1}
    \end{align*}
    利用公式：\( K_n(1, 1, \cdots, 1) = F_{n+1} \)，令所有 \( x = 1 \)：
    \begin{gather*}
...
\begin{problem}{6.8}{1}
  利用斐波那契数系将 65 英里/小时转换成千米/小时的近似值。
\end{problem}

由公式：
\[
  \boxed{F_{n+1} \ \text{千米} \ \approx F_n \ \text{英里}}
\]

\begin{gather*}
  \because 65 = 55 + 8 + 2 \\
  \therefore 89 + 13 + 3 = 105 (\text{千米})
\end{gather*}
...
\begin{problem}{6.11}{1}
  当 \( n \) 是一个非负整数时，带有交错符号的斯特林轮换数三角形的行和 \( \sum_k (-1)^k \genfrac{[}{]}{0pt}{} {n}{k} \) 等于什么？
\end{problem}

由公式：
\[
  \boxed{x^{\overline{n}} = \sum_k \genfrac{[}{]}{0pt}{} {n}{k} x^k \ , \quad \text{整数} \ n \ge 0}
\]
...
由公式：
\[
  \boxed{\sum_k \genfrac{[}{]}{0pt}{} {n}{k} \genfrac{\{}{\}}{0pt}{} {k}{m} (-1)^{n-k} = [m=n] \ , \quad \text{整数} \ m,n \ge 0}
\]
\begin{align*}
...
\begin{problem}{6.13}{1}
  在第 2 章和第 5 章里提到过微分算子 \( D=\dfrac{d}{dz} \) 以及 \( \vartheta = zD \)。我们有：
  \[
...
\begin{enumerate}
  ...
  \item[3.] 则对于 \( n+1 \) 时：
    ...
    又
    \begin{align*}
      &\phantom{=} \sum_k \genfrac{\{}{\}}{0pt}{} {n}{k} k z^{k} D^k + \sum_k \genfrac{\{}{\}}{0pt}{} {n}{k} z^{k+1} D^{k+1} \\
...
由习题~\ref{problem:6.12}~的反演定律，可知：对于 \( z^n D^n \) 公式也成立。

\begin{problem}{6.14}{1}
  证明关于欧拉数的幂恒等式：
  \[
    x^n = \sum_k \genfrac{\langle}{\rangle}{0pt}{} {n}{k} \binom{x+k}{n} \ , \ \text{整数} \ n \ge 0
  \]
\end{problem}
...
\begin{enumerate}
  ...
  \item[3.] 则对于 \( n+1 \)：
    ...
    所以
    \begin{align*}
      x^{n+1} &= \sum_k \genfrac{\langle}{\rangle}{0pt}{} {n}{k} \left( (k+1) \binom{x+k}{n+1} + (n-k) \binom{x+k+1}{n+1} \right) \\
...
\begin{problem}{6.15}{1}
  对
  \[
    x^n = \sum_k \genfrac{\langle}{\rangle}{0pt}{} {n}{k} \binom{x+k}{n} \ , \quad \text{整数} \ n \ge 0
  \]
  取 \( m \) 次差分来证明欧拉恒等式：
  \[
...
用算子表示差分：\( E \) 为移位算子，\( 1 \) 为恒等算子
\begin{gather*}
  \Delta^n = (E-1)^n = \sum_k \binom{n}{k} E^k (-1)^{n-k} \\
  \Rightarrow \Delta^m (x^n) = \sum_k \binom{m}{k} (x+k)^n (-1)^{m-k} \\
  \text{由公式：} \ \boxed{\Delta \left( \binom{x+k}{n} \right) = \binom{x+k}{n-1}} \\
  \text{因此：} \ \Delta^m \left( \sum_k \genfrac{\langle}{\rangle}{0pt}{} {n}{k} \binom{x+k}{n} \right) = \sum_k \genfrac{\langle}{\rangle}{0pt}{} {n}{k} \binom{x+k}{n-m}
\end{gather*}
...
\begin{problem}{6.16}{1}
  当 \( k \) 和 \( n \) 取遍所有整数的集合时，双重递归式
  \begin{align*}
    A_{n,0} &= a_n [n \ge 0] \ ; \quad A_{0,k} = 0 \ , \quad k > 0 \\
    A_{n,k} &= k A_{n-1,k} + A_{n-1,k-1} \ , \quad k, n \ \text{是整数}
  \end{align*}
  的通解是什么？
\end{problem}
...
\[
  \boxed{\text{第二类斯特林数：} \ \genfrac{\{}{\}}{0pt}{} {n}{k} = k \genfrac{\{}{\}}{0pt}{} {n-1}{k} + \genfrac{\{}{\}}{0pt}{} {n-1}{k-1}}
\]
...
\begin{problem}{6.19}{1}
  证明广义斯特林数满足：
  \begin{align*}
    & \sum_{k=0}^n \genfrac{\{}{\}}{0pt}{} {x+k}{x} \genfrac{[}{]}{0pt}{} {x}{x-n+k} (-1)^k \bigg/ \binom{x+k}{n+1} = 0 \ , \quad \text{整数} \ n > 0 \\
    & \sum_{k=0}^n \genfrac{[}{]}{0pt}{} {x+k}{x} \genfrac{\{}{\}}{0pt}{} {x}{x-n+k}  (-1)^k \bigg/ \binom{x+k}{n+1} = 0 \ , \quad \text{整数} \ n > 0
  \end{align*}
\end{problem}
...
和
\[
  \boxed{\genfrac{[}{]}{0pt}{} {n}{m} = \frac{n!}{(m-1)!} \sigma_{n-m} (n)}
\]

原式可化为：
\begin{align*}
  &\phantom{=} \sum_{k=0}^n \dfrac{(-1)^{2k+1} (x+k)! \ x! \ \sigma_{k} (-x) \ \sigma_{n-k} (x) \ (n+1)! (x+k-n-1)!}{(x-1)! (x-n+k-1)! (x+k)!} \\
...
\begin{problem}{6.26}{1}
  利用分部求和法计算 \( S_n = \sum_{k=1}^n H_k / k \)。\text{提示：}也考虑有关的和式 \( \sum_{k=1}^n H_{k-1} / k \).
\end{problem}
...
下证：\( F_{n+1} \ \bot \ F_n \)（互素），假设 \( a = \gcd{(F_{n+1}, F_n)} > 1 \)，因为 \( F_{n+1} = F_n + F_{n-1} \)，又 \( a \setminus F_{n+1} \) 且 \( a \setminus F_n \)（整除），所以
\begin{gather*}
  a \setminus F_{n-1} \\
  \vdots \\
  a \setminus F_1 = 1
...
利用算术基本定理，可以证明：
\[
  \gcd{((F_{m-n} F_{n+1}), F_n)} = \gcd{(F_{m-n}, F_n)} \ , \quad \text{当} \ F_{n+1} \ \bot \ F_n
\]
（具体证法可以参见习题~\ref{problem:4.38}）
...
\begin{problem}{6.28}{1}
  卢卡斯数 \( L_n \) 定义成 \( F_{n+1} + F_{n-1} \)。按照 \( F_{2n} = F_n F_{n+1} + F_{n-1} F_n \)，我们就有 \( F_{2n} = F_n L_n \)。
  \begin{enumerate}
    \item [a] 利用成套方法证明：一般递归式
      \[
        Q_0 = \alpha \ ; \quad Q_1 = \beta \ ; \quad Q_n = Q_{n-1} + Q_{n-2} \ , \quad n > 1
      \]
      的解 \( Q_n \) 可以用 \( F_n \) 以及 \( L_n \) 来表示。
    \item[b] 对 \( L_n \) 求一个用 \( \phi \) 以及 \( \hat{\phi} \) 来表示的封闭形式。
  \end{enumerate}
\end{problem}
...
a. 可以设 \( Q_n = A(n) \alpha + B(n) \beta \)：不妨令 \( \alpha = 1 \ , \ \beta = 0 \)，则 \( A(n) = A(n-1) + A(n-2) \)，同样的，令 \( \alpha = 0 \ , \alpha = 1 \)，得到：\( B(n) = B(n-1) + B(n-2) \)，因此 \( A(n) \) 与 \( B(n) \) 都与 \( F_n \) 相关。

又因为：\( Q_0 = \alpha \ , \ Q_1 = \beta \)，因此有：\(
  \begin{cases}
    A(0) = 1 \\
    A(1) = 0
\end{cases} \)
...
\begin{enumerate}
  \item[1.] 基础情况：当 \( k=0 \) 时：
    \begin{align*}
      \text{要证：} \ K_{m+n}(&x_1, \cdots, x_{m+n}) \\
    ...
    当 \( k=1 \) 时：
    \begin{align*}
      \text{要证：} \ K_{m+n}(&x_1, \cdots, x_{m+n}) \ (x_{m+1}) \\
      = &K_{m+1}(x_1, \cdots, x_{m+1}) K_n(x_{m+1}, \cdots, x_{m+n}) \\
      &+ (-1) K_{m-1}(x_1, \cdots, x_{m-1}) K_{n-2}(x_{m+3}, \cdots, x_{m+n}) \\
      \text{即证：} \ K_m(&x_1, \cdots, x_m) K_n(x_{m+1}, \cdots, x_{m+n}) \ (x_{m+1}) \\ +
      & K_{m-1}(x_1, \cdots, x_{m-1}) K_{n-1}(x_{m+2}, \cdots, x_{m+n}) \ (x_{m+1}) \\
      &= K_{m+1}(x_1, \cdots, x_{m+1}) K_n(x_{m+1}, \cdots, x_{m+n}) \\
      & \phantom{+} -  K_{m-1}(x_1, \cdots, x_{m-1}) K_{n-2}(x_{m+3}, \cdots, x_{m+n}) \\
      \text{即证：} \ K_n(&x_{m+1}, \cdots, x_{m+n}) \big(K_{m+1}(x_1, \cdots, x_{m+1}) - K_m(x_1, \cdots, x_m) (x_{m+1}) \big) \\
      = & K_{m-1}(x_1, \cdots, x_{m-1}) \big(K_{n-1}(x_{m+2}, \cdots, x_{m+n}) \ (x_{m+1}) \\
      \phantom{=} & + K_{n-2}(x_{m+3}, \cdots, x_{m+n}) \big) \\
      \text{即证：} \ K_n(&x_{m+1}, \cdots, x_{m+n}) K_{m-1}(x_1, \cdots, x_{m-1}) \\
      = & K_{m-1}(x_1, \cdots, x_{m-1}) K_n (x_{m+1}, \cdots, x_{m+n})
    \end{align*}
  \item[2.] 假设对于 \( k-1 \ , \ k \) 等式成立。
  \item[3.] 则对于 \( k+1 \)：
    \begin{align*}
...
先证明：
\begin{align*}
  &\phantom{=} K_n(x_1, \cdots, x_{n-1}, x_n + y) \\
...
推广：
\begin{align*}
  &\phantom{=} K(x_1, \cdots, x_{m-1}, x_m+y, x_{m+1}, \cdots, x_n) \\
  &= K(x_1, \cdots, x_m+y) K(x_{m+1}, \cdots, x_n) \\
  &\phantom{=} + K(x_1, \cdots, x_{m-1}) K(x_{m+2}, \cdots, x_n) \\
...
\begin{problem}{6.31}{2}
  对于用下降幂给出的上升幂的表示法：
  \[
    x^{\overline{n}} = \sum_k \genfrac{|}{|}{0pt}{} {n}{k} x^{\underline{k}} \ , \quad \text{整数} \ n \geqslant 0
  \]
...
和
\[
  \boxed{x^{\overline{n}} = \sum_k \genfrac{[}{]}{0pt}{} {n}{k} x^k}
\]
有：
\[
  x^{\overline{n}} = \sum_k \genfrac{[}{]}{0pt}{} {n}{k} \sum_j \genfrac{\{}{\}}{0pt}{} {k}{j} x^{\underline{j}}
\]
...
\begin{problem}{6.32}{2}
  在第 5 章里，我们用两种方法展开递归式 \( \displaystyle\binom{n}{k} = \binom{n-1}{k} + \binom{n-1}{k-1} \)，得到了公式：
  \[
...
\begin{problem}{6.33}{2}
  \( \displaystyle\genfrac{\{}{\}}{0pt}{} {n}{2} = (2^{n-1} - 1) [n > 0] \ , \ \ \genfrac{[}{]}{0pt}{} {n}{2} = (n-1)! \ H_{n-1} [n > 0] \)。对下一个情形 \( \displaystyle\genfrac{\{}{\}}{0pt}{} {n}{3} \) 和 \( \displaystyle\genfrac{[}{]}{0pt}{} {n}{3} \)，其封闭形式（不包含斯特林数）是什么？
\end{problem}
...
由习题~\ref{problem:6.26}：
\begin{align*}
  &= (n-1)! (H_{n-1}^2 - S_{n-1}) \\
  &= \dfrac{1}{2} (n-1)! \ (H_{n-1}^2 - H_{n-1}^{(2)})
\end{align*}

\begin{problem}{6.34}{2}
  如果假设基本递推关系
  \[
    \genfrac{<}{>}{0pt}{} {n}{k} = (k+1) \genfrac{<}{>}{0pt}{} {n-1}{k} + (n-k) \genfrac{<}{>}{0pt}{} {n-1}{k-1} \ , \quad \text{整数} \ n > 0
  \]
...
\begin{align*}
  ...
  &= \sum_{j=0}^k (k+1-j)^{-2} \quad \text{（上指标反转得到）} \\
  &= H_{k+1}^{(2)}
\end{align*}

\begin{problem}{6.35}{2}
  证明，对每个 \( \varepsilon > 0 \) 都存在一个整数 \( n > 1 \)（与 \( \varepsilon \) 有关），使得 \( H_n \bmod 1 < \varepsilon \).
\end{problem}

\( H_n \bmod 1 = H_n - \lfloor H_n \rfloor \)，要让 \( H_n - \lfloor H_n \rfloor < \varepsilon \)，而当 \( \lfloor H_n \rfloor > \lfloor H_{n-1} \rfloor \) 时：
\begin{align*}
  \because H_{n-1} < \lfloor H_{n-1} \rfloor + 1 = \lfloor H_n \rfloor \\
  \therefore H_n - \lfloor H_n \rfloor < H_n - H_{n-1} = \frac{1}{n} \\
\end{align*}

只要让此时：\( \frac{1}{n} < \varepsilon \)，即：\( n > \frac{1}{\varepsilon} \)，因此，\( n \) 满足：\( n > \dfrac{1}{\varepsilon} \) 并且 \( \lfloor H_n \rfloor > \lfloor H_{n-1} \rfloor \).

\begin{problem}{6.37}{1}
  用调和数表示 \( \sum_{k=1}^{mn} (k \bmod m) / k (k+1) \)，假设 \( m \) 和 \( n \) 是正整数，当 \( n \to \infty \) 时，其极限值是什么？
\end{problem}

\begin{align*}
  &\phantom{=} \sum_{k=1}^{mn} \dfrac{(k \bmod m)}{k (k+1)} \\
...
所以：
\begin{align*}
  &\phantom{=} \sum \binom{r}{k} (-1)^k H_k \delta k \\
...
原式等于
\begin{align*}
  &\phantom{=} \sum_{j=1}^n \frac{1}{j} ((n+1)H_{n+1} - jH_j - n - 1 + j) \\
  &= (n+1)H_{n+1}H_n - (n+1)H_{n+1} + (n+1) - (n+1)H_n + n \\
  &= (n+1) H_n^2 - (2n+1) H_n + 2n
\end{align*}

\begin{problem}{6.41}{2}
...
数学归纳法证明：
\begin{enumerate}
  \item[1.] 基础情况：\( n = 1 \) 时，\( S_1 = 1 = F_1 \)；当 \( n = 2 \) 时，\( S_2 = 1 = F_2 \)
  \item[2.] 假设 \( \{ 1, 2, \cdots, n \} \) 有 \( S_n \) 个子集具有此性质，对于 \( \{ 1, 2, \cdots, n+1 \} \) 有 \( S_{n+1} \) 个子集具有性质。
  \item[3.] 则对于 \( \{ 1, 2, \cdots, n+2 \} \) 具有此性质的子集个数为：

    在 \( S_n \) 的每一种取法上加上 \( n+2 \)，则：\( S \cup (S+1) = \{ 1, 2, \cdots, n+3 \} \)

    在 \( S_{n+1} \) 的每一种取法上加上 \( n+2 \)，则：\( S \cup (S+1) = \{ 1, 2, \cdots, n+3 \} \)

    因为具有此性质的子集必定包含 \( n+2 \)，故所有情况都已包含，且 \( S_{n+1} \) 中必定包含 \( n+1 \) 而 \( S_n \) 中必定没有这一项，因此没有重复。故 \( S_{n+2} = S_n + S_{n+1} \).
\end{enumerate}
...
\[
  ...
  \end{cases}
\]
且 \( A(n) \) 和 \( B(n) \) 与 \( F(n) \) 有关系，所以 \( A(n) = F_{n-1} \ , \quad B(n) = F_n \)
...
\]
所以 \( F_p \equiv \sum_{k} \binom{p}{2k+1} 5^k \pmod{p} \)。当 \( p = 2 \) 时，\( F_2 \bmod 2 = 1 \)

此外，注意到只有当 \( 2k+1 = p \)，即 \( k = \dfrac{p-1}{2} \) 时，有 \( \binom{p}{2k+1} = 1 \)，而其他时候 \( \binom{p}{2k+1} \bmod p = 0 \)，因此：\( F_p \equiv 5^{(p-1)/2} \pmod{p} \)

又只有当 \( k = 0 \) 和 \( k = \dfrac{p-1}{2} \) 时，\( \binom{p+1}{2k+1} \bmod p = 1 \)，而其他时候都有 \( \binom{p+1}{2k+1} \bmod p = 0 \)，因此：\( 2 F_{p+1} \equiv 1 + 5^{(p-1)/2} \pmod{p} \)

\begin{problem}{6.48}{2}
...
左等式等于
\begin{align*}
  &\phantom{=} K_m (x_1, \cdots, x_{m-1}, 0) K_{n-m} (x_{m+1}, \cdots, x_n) \\
  &\phantom{=} + K_{m-1} (x_1, \cdots, x_{m-1}) K_{n-m-1} (x_{m+2}, \cdots, x_n) \\
  &= K_{m-2} (x_1, \cdots, x_{m-2}) K_{n-m} (x_{m+1}, \cdots, x_n) \\
  &\phantom{=} + K_{m-1}(x_1, \cdots, x_{m-1}) K_{n-m-1}(x_{m+2}, \cdots, x_n) \\
  \xrightarrow{\text{简记为}} & K_{(1, m-2)} K_{(m+1, n)} + K_{(1, m-1)} K_{(m+2, n)}
\end{align*}

右等式等于
\begin{align*}
  &\phantom{=} K (x_1, \cdots, x_{m-1} + x_{m+1}) K_{(m+2, n)} \\
  &\phantom{=} + K(x_1, \cdots, x_{m-2}) K_{(m+3, n)} \\
  &= K_{(1, m-2)} \cdot (x_{m-1} + x_{m+1}) K_{(m+2, n)} \\
  &\phantom{=} + K_{(1, m-3)} K_{(m+2, n)} + K_{(1, m-2)} K_{(m+3, n)} \\
  &= (K_{(1, m-2)} x_{m-1} K_{(m+2, n)} + K_{(1, m-3)} K_{(m+2, n)}) \\
  &\phantom{=} + (K_{(1, m-2)} x_{m+1} K_{(m+2, n)} + K_{(1, m-2)} K_{(m+3, n)}) \\
...
\begin{problem}{6.53}{3}
  当 \( 0 \leqslant m \leqslant n \) 时，求 \( \displaystyle\sum_{k=0}^m \binom{n}{k}^{-1} (-1)^k H_k \) 的封闭形式。\textit{提示：}习题~\ref{problem:5.42}~有一个和式，它没有因子 \( H_k \).
\end{problem}

由习题~\ref{problem:5.42}~可知：
\[
...
因此：
\begin{align*}
  &\phantom{=} \displaystyle\sum_{k=0}^m \binom{n}{k}^{-1} (-1)^k H_k \\
...
因此：
\begin{align*}
  &\phantom{=} \sum_{n \geqslant 0} F_n^2 z^n \\
...
同理：
\begin{align*}
  &\phantom{=} \sum_{n \geqslant 0} F_n^3 z^n \\
...
\begin{problem}{6.77}{4}
  当 \( m \) 和 \( n \) 是整数，\( n \geqslant 0 \) 时，\( \sigma_n(m) \) 的值当 \( m < 0 \) 时由
  \[
    \genfrac{\{}{\}}{0pt}{} {n}{m} = (-1)^{n-m+1} \frac{n!}{(m-1)!} \sigma_{n-m}(-m)
  \]
  给出，当 \( m>n \) 时由
  \[
    \genfrac{[}{]}{0pt}{} {n}{m} = \frac{n!}{(m-1)!} \sigma_{n-m}(n)
  \]
  给出，而当 \( m=0 \) 时则由 \( \dfrac{B_m}{m!} = -m \sigma_{m}(0) \) 给出。证明：在其余情形，我们有：
  \[
    \sigma_n (m) = \dfrac{(-1)^{m+n-1}}{m! \ (n-m)!} \sum_{k=0}^{m-1} \genfrac{[}{]}{0pt}{} {m}{m-k} \dfrac{B_{n-k}}{n-k} \ , \ \text{整数} \ n \geqslant m > 0
  \]
\end{problem}
...
\begin{enumerate}
  ...
  \item[3.] 对于 \( m \)：
    ...
    \begin{align*}
      ...
      &= \dfrac{(m-1-n) (-1)^{m+n-2}}{(m-1)! (n-m+1)!} \Big( \sum_{k=0}^{m-1} \genfrac{[}{]}{0pt}{} {m-1}{m-k-1} \dfrac{B_{n-k}}{n-k} \\
      &\phantom{=+} - \genfrac{[}{]}{0pt}{} {m-1}{0} \dfrac{n-m+1}{n-m+1} \Big) \\
      ...
      &= \dfrac{(-1)^{m+n-1}}{(m-1)! \ (n-m)!} \sum_{k=0}^{m-1} \genfrac{[}{]}{0pt}{} {m}{m-k} \dfrac{B_{n-k}}{n-k} \\
      \Rightarrow
      \sigma_n (m) &= \dfrac{(-1)^{m+n-1}}{m! \ (n-m)!} \sum_{k=0}^{m-1} \genfrac{[}{]}{0pt}{} {m}{m-k} \dfrac{B_{n-k}}{n-k}
    \end{align*}
\end{enumerate}
\hfill \( \square \)
